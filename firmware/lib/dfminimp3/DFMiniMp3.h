...
public:
    ...
    uint16_t loop()
    {
        while (Serial.available() >= DfMp3_Packet_SIZE)
        {
...
private:
    ...
    void sendPacket(uint8_t command, uint16_t arg = 0, uint16_t sendSpaceNeeded = c_msSendSpace)
    {
        ...
        Serial.write(out, DfMp3_Packet_SIZE);
    ...
    bool readPacket(uint8_t* command, uint16_t* argument)
    {
        ...
        *argument = 0;
        ...
        do
        {
            ...
            read = Serial.readBytes(&(in[DfMp3_Packet_StartCode]), 1);
            if (read != 1)
            {
                // nothing read
                return false;
        ...
        read += Serial.readBytes(in + 1, DfMp3_Packet_SIZE - 1);
